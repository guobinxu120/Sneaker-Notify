...
class MYSQL_Pipeline(object):
  ...
  #
	# 	# Insert item into sneakersVooberlin table.
	# 	elif isinstance(item, SneakersVooberlinItem):
	# 		self.cursor.execute("INSERT INTO sneakersvooberlin (name, link, date) VALUES (%s, %s, %s)", (item['name'].encode('utf-8'), item['link'].encode('utf-8'), DATE))
  #
	# 	# Insert item into footwear table.
	# 	elif isinstance(item, FootwearItem):
	# 		self.cursor.execute("INSERT INTO footwear (name, link, date) VALUES (%s, %s, %s)", (item['name'].encode('utf-8'), item['link'].encode('utf-8'), DATE))
  #
	# 	# Insert item into shoes164 table.
	# 	elif isinstance(item, Shoes164Item):
	# 		self.cursor.execute("INSERT INTO shoes164 (name, link, date) VALUES (%s, %s, %s)", (item['name'].encode('utf-8'), item['link'].encode('utf-8'), DATE))
  #
	# 	# Insert item into footwearuomo table.
	# 	elif isinstance(item, FootwearUomoItem):
	# 		self.cursor.execute("INSERT INTO footwearuomo (name, link, date) VALUES (%s, %s, %s)", (item['name'].encode('utf-8'), item['link'].encode('utf-8'), DATE))
  #
	# 	# Insert item into sneakerSusi table.
	# 	elif isinstance(item, SneakerSusiItem):
	# 		self.cursor.execute("INSERT INTO sneakersusi (name, link, date) VALUES (%s, %s, %s)", (item['name'].encode('utf-8'), item['link'].encode('utf-8'), DATE))
  #
	#
	# 	self.conn.commit()

		# If item name contain below words. Tweet it.
		keywords = ['air','ultra boost', 'air jordan', 'jordan retro', 'nmd', 'boost', 'retro', 'flyknit', 'yeezy', 'ronnie', 'fieg', 'pharrel', 'atmos', 'clots', 'mars', 'yard']
		self.count += 1
		...
		if any(keyword in item['name'].encode('utf-8').lower() for keyword in keywords):
			...
			requests.post('https://hooks.slack.com/services/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', json={'text': "*" + item['name'] + "*" + "\n" + item['link'] + "\n" + item['size'] + "\n" + "------------" + "\n"}, headers={'Content-Type': 'application/json'})
