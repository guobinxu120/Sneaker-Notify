...
#new
SneakersVooberlinURL = "https://www.vooberlin.com/men/footwear/sneakers"
FootwearURL = "http://www.oneblockdown.it/it/footwear"
Shoes164URL = "https://www.antonia.it/164-shoes"
FootwearUomoURL = "https://www.holypopstore.com/it/footwear-uomo"
SneakerSusiURL = "https://www.susi.it/it-IT/uomo/scarpe/sneaker"
	...
	def parse(self, response):
		try:
			...
			for product in products:
				item = JDSportsItem()
				try:
					item['name'] = product.xpath('.//span/a/picture/img/@title').extract()[0]
				except:
					item['name'] = product.xpath('.//span/a/span/picture/img/@title').extract()[0]
...
class FootPatrolSpider(Spider):
	...
	bucket_count = 0
	pagenum = 1
	latest_response_product_id_list = []
	...
	def parse(self, response):
		...
		for product in products:
			...
			# 			  callback=self.parseIndividualProduct,
			# 			  meta={'item': item},
			# 			  dont_filter=True,
			# 			 )
			i += 1
		...
		next_url = response.xpath('//a[@class="search-results-load-more"]/@data-uri-template').extract_first()
		if next_url:
			self.navigation_url = next_url
		self.pagenum += 1
		next_url = self.navigation_url.replace('[[PAGENUM]]', str(self.pagenum))

		yield Request('https://www.footpatrol.co.uk' + next_url+'?ajax=1&ajax_paging=1', callback=self.parse)
		# else:
		# 	yield Request(FootPatrolURL, callback=self.parse, dont_filter=True, priority=26)
	def parseIndividualProduct(self, response):
		product_id = response.xpath('//*[@id="product_id"]/@value').extract_first()
		attribute_value_list = response.xpath('//*[@class="attribute_value_list"]/li/a')

		attr_str_data = []
		for attribute_value in attribute_value_list:
			size_float = 0.0
			try:
				size_temp_id = attribute_value.xpath('@href').extract_first()
				size_float = float(attribute_value.xpath('text()').re(r'[\d.]+')[0])
				attr_str_data.append(size_float)
			except:
				continue

			formdata = {}
			formdata['attributes[2]'] = str(size_temp_id)
			formdata['product_quantity'] = '1'
			formdata['submit'] = ''
			formdata['product_id'] = product_id
			formdata['show_image_variations'] = '0'
			formdata['product_thumbnails_pagination'] = '10'

			self.total_count += 1

			yield FormRequest('https://www.footpatrol.com/basket', self.parseBucket,
							  method="POST", formdata=formdata,
							  dont_filter=True,
							  meta={'product_id': product_id, 'size_float': size_float})

		self.products_data[product_id] = {}
		self.products_data[product_id]['item'] = response.meta['item']
		self.products_data[product_id]['attr_values'] = attr_str_data
		self.products_data[product_id]['skus'] = []

	def parseBucket(self, response):
		self.bucket_count += 1

		self.latest_response_product_id_list.append(response.meta['product_id'])

		print "Total_count : " + str(self.total_count)
		print "bucket_count : " + str(self.bucket_count)

		try:
			dataObject_str = response.xpath('//*[@type="text/javascript"]/text()').extract_first().replace('\n', '').replace('\t', '').strip()
			dataObject_str = dataObject_str.replace(';', '')
			dataObject_str = str(dataObject_str).split('= ')[1]
		except:
			return
		...
		for temp_str in dataObject_str.split('items: [')[1].split('},{'):
			sku = ''
			title = ''
			for temp_str1 in temp_str.split(','):
				temp_str1 = temp_str1.replace('{', '').replace('}', '').replace(']', '')

				l = temp_str1.split(':')
				if l[0].strip() == 'sku':
					sku = l[1].strip().replace("'", '')
				elif l[0].strip() == 'description':
					title = l[1].strip().replace("'", '')

			# product_id = self.latest_response_product_id_list[i]
			try:
				if not int(sku) in self.products_data[product_id]['skus']:
		...
		if self.total_count == self.bucket_count:
			# dataObject_str = response.xpath('//*[@type="text/javascript"]/text()').extract_first().replace('\n', '').replace('\t', '').strip()
			# dataObject_str = dataObject_str.replace(';', '')
			# dataObject_str = str(dataObject_str).split('= ')[1]
            #
			...
			for product_id in self.products_data.keys():
				item = self.products_data[product_id]['item']
				skus = self.products_data[product_id]['skus']
				attr_values = self.products_data[product_id]['attr_values']
				attr_values.sort()
				skus.sort()

				result_size_str = 'Sizes\n'

				for i, sku in enumerate(skus):
					result_size_str = result_size_str + '{}: {}.{}\n'.format(str(attr_values[i]).replace('.0', ''), product_id, str(sku))

				item['size'] = result_size_str
...
class SneakersVooberlinSpider(Spider):

	name = "SneakersVooberlinSpider"
	allowded_domains = ["vooberlin.com"]
	start_urls = [SneakersVooberlinURL]

	def __init__(self):
		logging.critical("SneakersVooberlinSpider STARTED.")

	def parse(self, response):
		products = Selector(response).xpath('//*[@class="item product product-item available"]/div')

		for product in products:
			item = SneakersVooberlinItem()

			names = product.xpath('div/h2/strong/a/text()').extract()
			if len(names) == 1:
				names = names[0].strip()
			else:
				new_names = []
				for name0 in names:
					new_names.append(name0.strip())
				names = '-'.join(new_names)

			item['name'] = names
			item['link'] = product.xpath('a/@href').extract_first()
			# item['image'] = product.xpath('a/img/@src').extract_first()
			# item['price'] = product.xpath('.//*[@class="price"]/text()').re(r'[\d.,]+')[0]
			item['size'] = '**NOT SUPPORTED YET**'
			yield item

		# yield Request(SneakersVooberlinURL, callback=self.parse, dont_filter=True, priority=104)

class FootwearSpider(Spider):

	name = "FootwearSpider"
	# allowded_domains = ["oneblockdown.it"]
	start_urls = [FootwearURL]

	def __init__(self):
		logging.critical("FootwearSpider STARTED.")
	def start_requests(self):
		scraper = cfscrape.create_scraper()
		response = scraper.get(FootwearURL).content
		yield Request(Shoes164URL, callback=self.parse, meta={'content':response})
		# cf_requests = []
	...
	def parse(self, response):
		response = TextResponse(url=response.url,
                                body=response.meta['content'],
                                encoding='utf-8')

		json_str = re.findall('var preloadedItems = \[(\{.*\})];',response.body)[0]
		json_str = json_str.replace('\\', '')#.replace('"Air Max 97 "Metallic Hematite""', '"Air Max 97 Metallic Hematite"').replace('"Obsidian"', 'Obsidian')
		json_str = '[{}]'.format(json_str)#.replace('","', "','").replace('":"', "':'").replace('"},"', "'},'").replace('"],"', "'],'").replace('",["', "',['").replace('",{"', "',{'").replace('",[{"', "',[{'").replace('"}],"', "'}],'").replace('"},{"', "'},{'").replace('"],["', "'],['").replace(',"', ",'").replace(',["', ",['").replace(',{"', ",{'").replace(',"', ",'").replace(',["', ",['").replace('":', ",{'")
		...
		for i in range(len(json_str)):
			if json_str[i] == ':':
				if json_str[i - 4 : i].lower() == 'http' or json_str[i - 5 : i].lower() == 'https':
					pass
				else:
					find_s = True
					continue
			if find_s:
				if json_str[i] == '"':
					...
					if n == 3:# and (json_str[i + 1].lower() in str_list):
						replace_str = json_str[index_list[len(index_list) - 2]: i + 1]
						replace_str_list.append(replace_str)
					else:
						if not (json_str[i + 1].lower() in str_list):
							find_s = False
							n = 0
							index_list = []

		for r in replace_str_list:
			json_str = json_str.replace(r, "'{}'".format(r[1: len(r) - 1]))

		print json_str
		json_data_list = loads(json_str, strict=False)
		n = 0
		for json_data in json_data_list:
			item = FootwearItem()

			item['name'] = json_data['title']
			item['link'] = json_data['permalink']


			# try:
			# 	item['image'] = json_data['coverObject']['imageUrl']
			# except:
			# 	try:
			# 		item['image'] = json_data['coverObject']['thumbUrl']
			# 	except:
			# 		item['image'] = ''
			# 		continue
			# item['price'] = json_data['displaySellPrice']
		...
		scraper = cfscrape.create_scraper()
		response = scraper.get(FootwearURL).content
		yield Request(Shoes164URL, callback=self.parse, meta={'content':response},  priority=105)
		# yield Request(FootwearURL, callback=self.parse, dont_filter=True, priority=105)

		# next_json_str = loads(re.findall('var preloadedPagination = (\{.*\});',response.body)[0])
		# if next_json_str['current'] < next_json_str['total']:
		# 	href = 'http://www.oneblockdown.it/it/footwear?page={}'.format(str(next_json_str['current'] + 1))
		# 	yield Request(href, callback=self.parse, dont_filter=True, priority=105)


class Shoes164Spider(Spider):

	name = "Shoes164Spider"
	allowded_domains = ["antonia.it"]
	start_urls = [Shoes164URL]

	def __init__(self):
		logging.critical("Shoes164Spider STARTED.")

	def parse(self, response):

		products = Selector(response).xpath('//*[@class="product_list grid row"]/li/div')

		for product in products:
			item = Shoes164Item()

			item['name'] = product.xpath('.//*[@class="product-name produ"]/text()').extract_first().strip()
			item['link'] = product.xpath('.//*[@class="product_img_link"]/@href').extract_first()
			# item['image'] = product.xpath('.//*[@class="product_img_link"]/img/@src').extract_first()
			# item['price'] = product.xpath('.//*[@itemprop="price"]/text()').re(r'[\d.,]+')[0]
			item['size'] = '**NOT SUPPORTED YET**'
			yield item
		# yield Request(Shoes164URL, callback=self.parse, dont_filter=True, priority=106)

class FootwearUomoSpider(Spider):

	name = "FootwearUomoSpider"
	allowded_domains = ["holypopstore.com"]
	start_urls = [FootwearUomoURL]

	def __init__(self):
		logging.critical("FootwearUomoSpider STARTED.")

	def parse(self, response):
		json_str = re.findall('var preloadedItems = \[(\{.*\})];',response.body)[0]
		json_str = json_str.replace('\\', '')#.replace('"Air Max 97 "Metallic Hematite""', '"Air Max 97 Metallic Hematite"').replace('"Obsidian"', 'Obsidian')
		json_str = '[{}]'.format(json_str)#.replace("'", ' ').replace('&apos;', '')
		json_str = json_str.replace(':""', '-----')
		...
		for json_data in json_data_list:
			...
			n += 1
			yield item
		# yield Request(FootwearUomoURL, callback=self.parse, dont_filter=True, priority=107)
		# next_page_xpaths = response.xpath('//*[@id="pagination"]/ul/li')
		# if next_page_xpaths:
		# 	try:
		# 		next_page_number = response.meta['next_page_number']
		# 		response.meta['next_page_number'] += 1
		# 	except:
		# 		response.meta['next_page_number'] = 2
		# 	for next_page_xpath in next_page_xpaths:
		# 		num = next_page_xpath.xpath('a/text()').extract_first()
		# 		if num:
		# 			try:
		# 				num = int(num)
		# 			except:
		# 				break
        #
		# 			if num == response.meta['next_page_number']:
		# 				href = next_page_xpath.xpath('a/@href').extract_first()
		# 				yield Request(href, callback=self.parse, dont_filter=True, meta=response.meta)

class SneakerSusiSpider(Spider):

	name = "SneakerSusiSpider"
	allowded_domains = ["susi.it"]
	start_urls = [SneakerSusiURL]

	scrol_count = 0

	def __init__(self):
		pass
		# logging.critical("SneakerSusiSpider STARTED.")

	def parse(self, response):
		products = Selector(response).xpath('//*[@class="product-tile"]')
		...
		if products:
			for product in products:
				item = SneakerSusiItem()

				name1 = product.xpath('.//*[@class="prod-header"]/h3/a/text()').extract_first().strip()
				name2 = product.xpath('.//*[@class="prod-header"]/p/a/text()').extract_first().strip()

				item['name'] = '{}-{}'.format(name1, name2)
				item['link'] = product.xpath('.//*[@class="prod-header"]/h3/a/@href').extract_first()
				# item['image'] = SneakerSusiURL + product.xpath('.//*[@class="product-image"]/a/img/@src').extract_first()
				# item['price'] = product.xpath('.//*[@class="product-sales-price"]/text()').re(r'[\d.,]+')[0].replace(',', '.')
				item['size'] = '**NOT SUPPORTED YET**'
				yield item

			self.scrol_count += 1

			temp_url = 'https://www.susi.it/it-IT/uomo/scarpe/sneaker/?sz=12&start={}&format=page-element'.format(str(12 * self.scrol_count))

		# yield Request(SneakerSusiURL, callback=self.parse, dont_filter=True, priority=108)
#
#
#
...
#
# #new
# process.crawl(SneakersVooberlinSpider)
# process.crawl(FootwearSpider)
# process.crawl(Shoes164Spider)
# process.crawl(FootwearUomoSpider)
# process.crawl(SneakerSusiSpider)
